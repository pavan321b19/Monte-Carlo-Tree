...
##Time taken comparision
df_new = df[df['Pacman Agent'] == 'ExpectimaxAgent']
expectiaxTime = df_new['Time Taken'].tolist()
expectiaxTimeAvg = sum(expectiaxTime)/len(expectiaxTime)

df_new = df[df['Pacman Agent'] == 'MinimaxAgent']
minimaxTime = df_new['Time Taken'].tolist()
minimaxTimeAvg = sum(minimaxTime)/len(minimaxTime)

df_new = df[df['Pacman Agent'] == 'MCTSAgent']
mctsTime = df_new['Time Taken'].tolist()
mctsTimeAvg = sum(mctsTime)/len(mctsTime)

agentList = ["Minimax/Expectimax/MCTS Agents"]
layoutList = ["Minimax","Expectimax","MC Tree Search"]
ylabel ='Time Taken'
title ='Time Taken Comparision'
generatePlot(minimaxTimeAvg,expectiaxTimeAvg,mctsTimeAvg/2,agentList,layoutList,ylabel,title,"TimeTaken")
